...
<main class="page-content row bg-body">
    <section class="page-header bg-body">
        ...
        <ul class="breadcrumb bg-body s-20">
            ...
            <li><a href="{{ URL::to('pdm/lineasaccion?id='.$id.'&idtema='.$idtema.'&idps='.$idps.'&idpo='.$idpo.'&idpe='.$idpe) }}">Líneas de Acción</a></li>
            <li class="active"><a href="#">Metas</a></li>
        </ul>
    </section>
    <div class="toolbar-line">
        <div class="col-md-12 m-b-md">
            <button type="button" onclick="location.href='{{ URL::to('pdm/lineasaccion?id='.$id.'&idtema='.$idtema.'&idps='.$idps.'&idpo='.$idpo.'&idpe='.$idpe) }}'" class="btn btn-default btn-xs b-r-30"><i class="fa fa-arrow-circle-left"></i> Regresar</button>
            <a href="#" class="tips btn btn-sm c-white b-r-30 btnagregar" style="background: var({{ $row->color }});" title="Agregar Meta"><i class="fa fa-plus-circle"></i>&nbsp;Agregar Meta</a>
        </div>
    </div>
        ...
        <article class="col-sm-12 col-md-12 col-lg-12">
                ...
                <div class="sbox animated fadeInRight" style="border-left:4px solid var({{ $row->color }});">
                    ...
                    <div class="sbox-content">
                        <div class="col-sm-12 col-md-12 col-lg-12">
                            ...
                            <div class="col-sm-12 col-md-12 col-lg-12 p-xs">
                                <label class="control-label col-sm-3 col-md-3 col-lg-3 text-right s-16"> LÍNEA DE ACCIÓN: </label>
                                <div class="col-sm-8 col-md-8 col-lg-8 s-16 c-text-alt">{{ $row->linea_accion }}</div>
                            </div>
                        </div>
            ...
            <section class="col-sm-7 col-md-7 col-lg-7 p-md">
                <div class="col-sm-12 col-md-12 col-lg-12 bg-white border-gray p-md b-r-c" id="line-comm">
                    <div class="col-sm-12 col-md-12 col-lg-12 c-text s-16 b-b-gray p-xs">METAS</div>
...
query();
function query(){
    axios.get('{{ URL::to("pdm/searchmetas") }}',{params:{idpla:idpla}}).then(response=>{
        $("#result2").empty().append(response.data);
    })
}
$(".btnagregar").click(function(e){
    e.preventDefault();
    modalMisesa("{{ URL::to('pdm/agregarmeta') }}",{idpla:idpla},"Agregar Meta","50%");
})
$(document).on("click",".btnedit",function(e){
    e.preventDefault();
    modalMisesa("{{ URL::to('pdm/editarmeta') }}",{id:$(this).attr("id")},"Editar Meta","50%");
});
$(document).on("click",".btndestroy",function(e){
    e.preventDefault();
    let id=$(this).attr("id");
    swal({
        title:'Estás seguro de eliminar la meta?',
        icon:'warning',
        buttons:true,
        dangerMode:true
    }).then((willDelete)=>{
        if(willDelete){
            axios.delete('{{ URL::to("pdm/meta") }}',{params:{id:id}}).then(response=>{
