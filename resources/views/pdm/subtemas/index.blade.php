...
<main class="page-content row bg-body">
    <section class="page-header bg-body">
        <div class="page-title">
            <h3 class="s-20" style="color:var({{ $row->color }});">
                {{ $pageTitle }} <small class="s-16">{{ $pageNote }}</small>
            </h3>
        </div>
        <ul class="breadcrumb bg-body s-20">
            <li><a href="{{ URL::to('dashboard') }}"> Dashboard </a></li>
            <li><a href="{{ URL::to('pdm?id=' . $id) }}">{{ $pageTitle }}</a></li>
            <li class="active"><a href="{{ URL::to('pdm/subtemas?id=' . $id . '&idtema=' . $idtema) }}">Subtema</a></li>
        </ul>
    </section>

    <div class="toolbar-line">
        <div class="col-md-12 m-b-md">
            <button type="button" onclick="location.href='{{ URL::to('pdm?id=' . $id) }}' " class="btn btn-default btn-xs b-r-30">
                <i class="fa  fa-arrow-circle-left "></i> Regresar
            </button>
            <a href="#" class="tips btn btn-sm b-r-30 c-white btnagregar" style="background: var({{ $row->color }});" title="Agregar Subtema">
                <i class="fa fa-plus-circle"></i>&nbsp;Agregar Subtema
            </a>
        </div>
    </div>
        ...
        <article class="col-sm-12 col-md-12 col-lg-12">
            <section class="col-sm-5 col-md-5 col-lg-5 no-padding">
                <div class="sbox animated fadeInRight b-r-5" style="border-left:4px solid var({{ $row->color }});">
                    <div class="sbox-title">
                        <h4><i class="fa fa-table"></i> PDM</h4>
                    </div>
            ...
            <section class="col-sm-7 col-md-7 col-lg-7 p-md">
                <div class="col-sm-12 col-md-12 col-lg-12 border-gray p-md b-r-5 bg-white" id="line-comm">
                    <div class="col-sm-12 col-md-12 col-lg-12 c-text s-16 b-b-gray p-xs">SUBTEMAS</div>
...
<script>
    ...
    function query() {
        axios.get('{{ URL::to('pdm/searchsubtemas') }}', {
            params: {
                idtema: idtema,
                id: id,
                color: color
    ...
    $(".btnagregar").click(function(e) {
        e.preventDefault();
        modalMisesa("{{ URL::to('pdm/agregarsubtema') }}", { idtema: idtema }, "Agregar Subtema", "50%");
    })

    $(document).on("click", ".btnedit", function(e) {
        e.preventDefault();
        modalMisesa("{{ URL::to('pdm/editarsubtema') }}", { idps: $(this).attr("id") }, "Editar Subtema", "50%");
    });

    $(document).on("click", ".btndestroy", function(e) {
        e.preventDefault();
        let idps = $(this).attr("id");
        swal({
            ...
            if (willDelete) {
                axios.post('{{ URL::to('pdm/destroysubtema') }}', {
                    params: { idps: idps }
                }).then(response => {
