								...
								<td>
									<ul>
										<li v-for="r in info.rows"> 
											<i class="fa fa-trash-o c-danger cursor" @click.prevent="deleteYear(r.id)"></i> @{{ r.anio }}</li>
									</ul>
								</td>
...
<script>
     ...
     var years = new Vue({
		...
		methods:{
			rowsProjects(){
				axios.get('{{ URL::to($pageModule."/listmodules") }}',{
					params : {idi:this.idi},
				}).then(response => {
                    this.rowsData = [];
				...
				modalMisesa("{{ URL::to($pageModule.'/addmodule') }}",{idi:"{{ $idi }}",no:no},"Agregar Año","60%");
			},deleteYear(id){
				swal({
                  ...
                  if(willDelete){
                      axios.delete('{{ URL::to($pageModule."/yearmodule") }}',{
                          params : {id:id}
                      }).then(response => {
