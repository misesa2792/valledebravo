...
<script>
    ...
    var commets = new Vue({
        ...
        methods:{
            loadImgs(){
                axios.get('{{ URL::to("alineacion/comentariosimgar") }}',{
                    params : {idar:idar}
                }).then(response => {
                    this.infoimgs = [];
             ...
             destroyFile(idai){
                swal({
                    ...
                    if(willDelete){
                        axios.get('{{ URL::to("alineacion/destroyfilealineacionar") }}',{
                            params : {idai:idai}
                        }).then(response => {
    ...
    $("#saveInfo").on("submit", function(e){
      ...
      var formData = new FormData(document.getElementById("saveInfo"));
        $.ajax("{{ URL::to('alineacion/actividadesrelevanteseditar?idar='.$idar) }}", {
            type: 'post',
            dataType: "html",
            data: formData,
            cache: false,
