              ...
              <article class="col-md-12">
                ...
                <article class="col-md-12">
                      ...
                      <tr class="t-tr-s14 c-text-alt">
                                    ...
                                    <td width="60%">
                                      <select name="idproyecto" class="mySelect" id="sltproject" onchange="obtenerValorData('sltproject')" required>
                                        <option value="">--Select Please--</option>
                    ...
                    <div class="col-md-6">
                        <table class="table table-bordered bg-white">
                            ...
                            <tr class="t-tr-s14 c-text-alt">
                                ...
                                <td></td>
                                <td>
                                  <select name="iddep_gen_ext" class="mySelect"  required>
                                    <option value="">--Select Please--</option>
                                    @foreach ($data['rowsDepGen'] as $v)
                                      <option value="{{ $v->id }}" 
                                        @if($v->id == $data['row']->iddep_gen_ext) selected @endif>{{ $v->no_dep_gen.' '.$v->dep_gen }}</option>
                                    @endforeach
                                  </select>
                                </td>
                            </tr>
                            <tr class="t-tr-s14 c-text-alt">
                                ...
                                <td></td>
                                <td>
                                  <select name="iddep_aux_ext" class="mySelect"  required>
                                    <option value="">--Select Please--</option>
                                    @foreach ($data['rowsDepAux'] as $v)
                                      <option value="{{ $v->id }}" 
                                        @if($v->id == $data['row']->iddep_aux_ext) selected @endif>{{ $v->no_dep_aux.' '.$v->dep_aux }}</option>
                                    @endforeach
                                  </select>
                                </td>
                            </tr>
                            ...
                            <tr class="t-tr-s14 c-text-alt">
                                <th>Proyecto: </th>
                                <td id="td_no_proyecto_ext"></td>
                                <td width="60%">
                                  <select name="idproyecto_ext" class="mySelect" id="sltproject_ext" onchange="obtenerValorDataExt('sltproject_ext')" required>
                                    <option value="">--Select Please--</option>
                                    @foreach ($data['rowsProjects'] as $v)
                                      <option value="{{ $v->idproyecto }}" 
                                        data-idproyecto="{{ $v->idproyecto }}" 
                                        ...
                                        data-obj_programa="{{ $v->obj_programa }}" 
                                        data-clasificacion="{{ $v->clasificacion }}" 
                                        @if($v->idproyecto == $data['row']->idproyecto_ext) selected @endif>{{ $v->no_proyecto.' '.$v->proyecto }}</option>
                                    @endforeach
                                  </select>
                                </td>
                            </tr>
                            <tr class="t-tr-s14 c-text-alt">
                                <th>Programa presupuestario: </th>
                                <td id="td_no_programa_ext"></td>
                                <td id="td_programa_ext"></td>
                            </tr>
                            <tr class="t-tr-s14 c-text-alt">
                                <th>Objetivo: </th>
                                <td></td>
                                <td id="td_obj_programa_ext"></td>
                            </tr>
                        </table>
                    </div>
                </article>
...
<script>
  $(document).ready(function() { 
    ...
    function obtenerValorData(opcion) {
    ...
    function obtenerValorDataExt(opcion) {
        var select = document.getElementById(opcion);
        var opcionSeleccionada = select.options[select.selectedIndex];
        var no_programa = opcionSeleccionada.getAttribute("data-no_programa");
        var programa = opcionSeleccionada.getAttribute("data-programa");
        ...
        var no_proyecto = opcionSeleccionada.getAttribute("data-no_proyecto");
        var clasificacion = opcionSeleccionada.getAttribute("data-clasificacion");
        $("#td_no_programa_ext").html(no_programa);
        $("#td_programa_ext").html(programa);
        $("#td_obj_programa_ext").html(obj_programa);
        $("#td_no_proyecto_ext").html(no_proyecto);
    }
    function viewTr(){
