        ...
        <table class="table">
            <tr>
                ...
                <td class="no-borders text-center">
                    <div class="font-bold text-center s-16 c-black">H. AYUNTAMIENTO DE {{ $data['institucion'] }} </div>
                ...
                <table class="table">
                    ...
                    <tr>
                        ...
                        <td class="s-16 no-borders">
                            <input type="text" name="fecha" value="{{ $data['institucion'] }}, MÉXICO A {{ date('d') }} DE {{ mb_strtoupper(SiteHelpers::mes(date('n'))) }} DE {{ date('Y') }}" class="form-control border-b-1-dashed" placeholder="MUNICIPIO, MÉXICO A 00 DE 0000"  required>
                        </td>
                    </tr>
                </table>
            </div>
        ...
        <article class="col-md-12 m-t-md">
			<table class="table table-bordered">
				<tr class="t-tr-s14 c-text-alt">
					...
					<th class="bg-white text-center" width="20%">DE LA CANCELACIÓN O REDUCCIÓN DE METAS Y/O RECURSOS DEL PROYECTO (IMPACTO O RECUPERACIÓN PROGRAMÁTICA)</th>
					<th class="bg-white text-center" width="20%">DE LA CREACIÓN, REASIGNACIÓN DE METAS Y /O RECURSOS AL PROYECTO (BENEFICIO, IMPACTO, REPERCUCIÓN PROGRAMÁTICA)</th>
					<th class="bg-white text-center" width="20%">IDENTIFICACIÓN DEL ORIGEN DE LOS RECURSOS</th>
				</tr>
                    @foreach ($data['rowsRegistros'] as $key => $r)
                        <tr class="t-tr-s14 c-text-alt bg-white">
                            <td class="text-center">{{ $data['dep_ext']['clave'] }}</td>
                            <td class="text-center">{{ $data['dep_ext']['proyecto'] }}</td>
                            <td>
                            ...
                            <td>
                                <textarea name="identificacion" class="form-control" rows="5" placeholder="IDENTIFICACIÓN DEL ORIGEN DE LOS RECURSOS" required>Reducción de recursos del programa {{ $data['dep_int']['clave'] }}, de la partida {{ $r['d_partida'] }} por $ {{ $r['importe'] }} que se destinará a la partida {{ $r['a_partida'] }}. Fuente de financiamiento {{ $r['no_ff'] }}, {{ $r['ff'] }} </textarea>
                            </td>
                        </tr>
                    @endforeach
                    @for ($i = 0; $i < 5; $i++)
			...
			<table class="table">
                ...
                <tr class="t-tr-s14 c-text-alt">
                    <td width="33%" class="bg-white">
                        ...
                        <br>
                        <br>
                        <input type="text" value="" name="txt_dep_gen" class="form-control border-b-1-dashed text-center c-black font-bold" placeholder="DEP. GENERAL" onkeyup="MassMayusculas(this);" required>
                    ...
                    <td width="33%" class="bg-white">
                        ...
                        <br>
                        <br>
                        <input type="text" value="{{ $data['footer']['t_uippe'] }}" name="txt_uippe" class="form-control border-b-1-dashed text-center c-black font-bold" placeholder="TITULAR DE UIPPE O EQUIVALENTE" onkeyup="MassMayusculas(this);" required>
                        <div class="text-center c-text">NOMBRE Y FIRMA</div>    
                    </td>  
                    <td class="no-borders"></td>                   
                    <td width="33%" class="bg-white">
                        ...
                        <br>
                        <br>
                        <input type="text" value="{{ $data['footer']['t_tesoreria'] }}" name="txt_tesorero" class="form-control border-b-1-dashed text-center c-black font-bold" placeholder="AUTORIZÓ (ENCARGADA DEL DESPACHO DE LA TESORERÍA MUNICIPAL)" onkeyup="MassMayusculas(this);" required>
                        <div class="text-center c-text">NOMBRE Y FIRMA</div>    
                    </td>                     
                </tr>
			</table>
      ...
      swal({
        ...
        if(willDelete){
                ...
                formData.append("idam", "{{ $idam }}");
                    $.ajax("{{ URL::to($pageModule.'/generatepdfnota') }}", {
                        type: 'post',
                        dataType: "html",
                        data: formData,
                        cache: false,
