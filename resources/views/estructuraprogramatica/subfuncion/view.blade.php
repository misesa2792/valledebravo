    ...
    <table class="table table-bordered">
        ...
        @foreach ($rows as $v)
            <tr>
                ...
                <td class="c-text-alt s-16">{{ $v->no_subfuncion }}</td>
                <td class="c-text-alt s-16">{{ $v->subfuncion }}</td>
                <td class="text-center">
                    <a  href="{{ URL::to('estructuraprogramatica/subfuncion?idfinalidad='.$idfinalidad.'&idfuncion='.$idfuncion.'&idsubfuncion='.$v->id) }}" class="btn btn-xs btn-white no-borders" title="Abrir Objetivo"> 
