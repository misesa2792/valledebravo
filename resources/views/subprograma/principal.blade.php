                                              ...
                                              <tr>
                                                    <td>
                                                        ...
                                                        <select name="estatus" class="select2">
                                                            ...
                                                            @foreach ($rowsEstatus as $k_est => $v)
                                                                <option value="{{ $k_est }}" >{{ $v }}</option>
                                                            @endforeach
                                                        </select>
                                                    </td>
                                                  <td>
                                                      <div class="s-14 c-text-alt">No. Subprograma</div>
                                                      <input type="text" name="no_subprograma" class="form-control" placeholder="Número del subprograma">
                                                      <input type="hidden" name="page" value="1" id="pagep">
                                                  </td>
												  <td>
														<div class="s-14 c-text-alt">Subprograma</div>
														<input type="text" name="subprograma" class="form-control" placeholder="Nombre del subprograma">
													</td>
...
query();
    ...
    $(document).on("click",".btnedit",function(e){
        e.preventDefault();
        modalMisesa("{{ URL::to($pageModule.'/update') }}",{id:$(this).attr("id"), idy:$(this).attr("idy")}, "Subprograma",'50%');
    });
</script>		
@stop