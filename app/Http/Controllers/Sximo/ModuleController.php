...
class ModuleController extends Controller {
            ...
            if($form['field'] == $field_id && $form['alias'] == $alias)
            {
                ...
                $f = array(
                    ...
                    'option'        => array(
                        ...
                        'defaultval'              => isset( $form['option']['defaultval'])?$form['option']['defaultval']:'',
                        'tooltip'                => $tooltip ,
                        'attribute'                => $attribute,
                        'extend_class'            => isset( $form['option']['extend_class'])?$form['option']['extend_class']:''
                        ),
    ...
    public function postSavetable( Request $request)
    {
        ...
        for($i=1; $i<= $total ;$i++) {
            ...
            $grid[] = array(
                ...
                'formato'    => array(
                        'type'        => (isset($format_type[$i])) ? $format_type[$i] : '',
                        'decimals'    => (isset($format_decimals[$i])) ? $format_decimals[$i] : '',
                        'sep_decimal' => (isset($format_sep_decimal[$i])) ? $format_sep_decimal[$i] : '',
                        'pretext'    => (isset($format_pretext[$i])) ? $format_pretext[$i] : '',
                        'sep_thousand'    => (isset($format_sep_thousand[$i])) ? $format_sep_thousand[$i] : '',
                        'postext'    => (isset($format_postext[$i])) ? $format_postext[$i] : '',
                        'boolval'    => (isset($format_boolval[$i])) ? $format_boolval[$i] : '',
                        'dateformat'    => (isset($format_dateformat[$i])) ? : ''
                        ),
                'attribute'    => array(
                    'hyperlink'    => array(
   ...
   function getPermission( $id )
    ...
    function getFormat( Request $request , $id ) // forma para formatear el campo
    {
        $row = \DB::table('tb_module')->where('module_id', $id)
                                ->get();
        if(count($row) <= 0){
        ...
        foreach($config['grid'] as $form)
        {
            if($form['field'] == $field_id)
            {
                ...
                $f = array(
                    'type'        => (isset($form['formato']['type']) ? $form['formato']['type'] : 'text'),
                    'decimals'        => (isset($form['formato']['decimals']) ? $form['formato']['decimals'] : '2'),
                    'sep_decimal'        => (isset($form['formato']['sep_decimal']) ? $form['formato']['sep_decimal'] : '.'),
                    'pretext'        => (isset($form['formato']['pretext']) ? $form['formato']['pretext'] : ''),
                    'sep_thousand'    => (isset($form['formato']['sep_thousand']) ? $form['formato']['sep_thousand'] : ','),
                    'postext'        => (isset($form['formato']['postext']) ? $form['formato']['postext'] : ''),
                    'boolval'        => (isset($form['formato']['boolval']) ? $form['formato']['boolval'] : ''),
                    'dateformat'        => (isset($form['formato']['dateformat']) ? $form['formato']['dateformat'] : ''),
                    );
            }
        }
        ...
        $this->data['field_id'] = $field_id ;
        $this->data['alias'] = $alias;
        return view('sximo.module.formatfield',$this->data);
    }

    public function postFormat( Request $request ) // recibe el post del formato del campo
    {
        $id = $request->input('module_id');
        $field_id     = $request->input('field_id');
        $alias         = $request->input('alias');
            ...
            if($form['field'] == $field_id && $form['alias'] == $alias )
            {
                if($request->input('type') !='')
                {
                    $form['formato'] = array(
                        ...
                        'sep_decimal' => $request->input('sep_decimal'),
                        'pretext'    => $request->input('pretext'),
                        'sep_thousand'    => $request->input('sep_thousand'),
                        'postext'    => $request->input('postext'),
                        'boolval'    => $request->input('boolval'),
