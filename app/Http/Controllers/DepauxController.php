...
use App\Models\Catalogos\Depaux;
...
class DepauxController extends Controller {

    use JsonResponds;

	protected $layout = "layouts.main";
	protected $data = array();	
	public $module = 'depaux';
	static $per_page	= '10';
	protected $model;	
	protected $access;	
	...
	function getUpdate(Request $request)
	{
		...
		if($row)
		{
			...
			$this->data['row'] = $this->model->getColumnTable('ui_dep_aux'); 
		}
		$this->data['rowsTipo'] = $this->model->getCatTipoDependencia(); 
		$this->data['idy'] = $request->idy;
		$this->data['id'] = $id;
	...
	function postSave( Request $request)
	{
		...
		$id = empty($request->iddep_aux) ? 0 : $request->iddep_aux;
