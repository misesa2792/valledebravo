...
class depgen extends Sximo  {
	
	protected $table = 'ui_dep_gen';
	protected $primaryKey = 'iddep_gen';
	...
	public static function getSearch($request){
        ...
        $query = DB::table('ui_dep_gen as d')
            ->join('ui_tipo_dependencias as td', 'td.idtipo_dependencias', '=', 'd.idtipo_dependencias')
            ->select([
                'd.iddep_gen as id',
                'td.abreviatura',
                'd.numero as no_dep_gen',
                'd.descripcion as dep_gen',
            ])
            ->where('d.idanio', $idy);
