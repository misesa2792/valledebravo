...
class SiteHelpers
{
	//29-07-2025 - Checar para eliminar ya que en POA se esta formateando a solo aceptar 2 decimales (normalizeAmount) y ademas se valida que sea entero (formatCleanAmount)
	public static function getMassDecimales($numero=0) {
	...
	public static function mes($no=null) {
	...
	public static function Querygeneral($dato,$tabla,$clave,$id) {
		foreach( DB::select("SELECT ".$dato." FROM ".$tabla." where ".$clave."=".$id) as $k =>$p){
		  return $p->$dato;
    	}
	}
	public static function Acomodarfecha($fecha=null,$version=null) {
		if($fecha != null || $fecha != ""){
				if($version == 'asc'){
	...
	public static function avatarPersonal($idp,$width =60){
		$avatar = '<img src="'.asset('images/operadores').'/no-avatar.jpg"  width="'.$width.'"  height="'.$width.'"  class="b-r-100 border-white-4"/>';
		$Q = DB::table("rh_personal")->where("idpersonal",'=',$idp)->get();
		if(count($Q)>=1){
			$row = $Q[0];
			if($row->avatar !=''){
				$files =  './images/credencial/personal/'.$idp.'/'.$row->avatar;
				if(file_exists($files)){
					return  '<img src="'.asset('images/credencial/personal').'/'.$idp.'/'.$row->avatar.'"  width="'.$width.'"  height="'.$width.'"  class="b-r-100 border-white-4"/>';
				} else {
					return $avatar;
				}
			} else {
	...
	public static function avatar($width=80){
		$avatar = '<img src="'.asset('images').'/operadores/no-avatar.jpg"  width="'.$width.'"  height="'.$width.'" class="b-r-100 border-white-4"/>';
		return $avatar;
		/*$Q = DB::table("tb_users")->where("id",'=',Session::get('uid'))->get();
		if(count($Q)>=1){
			$row = $Q[0];
			$files =  './images/operadores/'.$row->avatar;
			if($row->avatar !='' ){
				if( file_exists($files)){
					return  '<img src="'.asset('images/operadores').'/'.$row->avatar.'"  width="'.$width.'"  height="'.$width.'" class="b-r-100 border-white-4"/>';
				} else {
					return $avatar;
				}
			} else {
	...
	public static function avatarUser($id=null,$tam=50)
{
	$avatar = '<a href="'.asset('images/operadores/no-avatar.jpg').'" target="_blank" class="previewImage">
							<img src="'.asset('images/operadores/no-avatar.jpg').'"  width="'.$tam.'"  height="'.$tam.'" style="border-radius:100%;border:2px solid var(--border-color);" />
							</a>';
	...
	if(count($Q)>=1)
	{
		...
		if($row->avatar !='' )
		{
			if( file_exists($files))
			{
				return  '<a href="'.asset('images/operadores').'/'.$row->avatar.'" target="_blank" class="previewImage">
									<img src="'.asset('images/operadores').'/'.$row->avatar.'" border="0" width="'.$tam.'" height="'.$tam.'" style="border-radius:100%;border:2px solid var(--color-success);" />
									</a>';
			} else {
				return $avatar;
			}
...
public static function Imagenes($file , $tam = 80 ) {
	return  '<a href="'.asset('images').'/'.$file.'" target="_blank" class="previewImage">
						<img src="'.asset('images').'/'.$file.'" border="0" width="'.$tam.'" height="'.$tam.'" style="border-radius:100%;border:1px solid #0d540b;" />
						</a>';
}
public static function Logos($file, $tam = 80 ,$width=200,$url=null) {
	return  '<a href="'.asset($url).'/'.$file.'" target="_blank" class="previewImage">
						<img src="'.asset($url).'/'.$file.'" border="0" width="'.$width.'" height="'.$tam.'"  />
						</a>';
}
	...
	public static function nestedMenu($parent=0,$position ='top',$active = '1'){
		...
		$Q = DB::select("SELECT tb_menu.* FROM tb_menu WHERE parent_id ='". $parent ."' ".$active." AND position ='{$position}' GROUP BY tb_menu.menu_id ORDER BY ordering	");
		return $Q;
	}
 ...
 public static function alphaID($in, $to_num = false, $pad_up = false, $passKey = null){
					...
					if($form['field'] !='entry_by'){
						...
						if($form['form_group'] == $i){
							if($display == 'horizontal'){
								...
								$f .= ' <div class="form-group '.$hidethis.' '.$inhide.'" '.$show .'><label for="ipt" class=" control-label "> '.$form['label'].'  '.$required.' '.$tooltip.' </label>'.self::formShow($form['type'],$form['field'],$form['required'],$form['option']).' </div> ';
							}
						}
					}
				}
	...
	public static function toView( $grids ){
		...
		foreach($grids as $grid){
			...
			if($grid['detail'] =='1'){
				...
				$f .= "<tr><td width='30%' class='label-view text-right'>".$grid['label']."</td><td>".$val." </td>	</tr>";
			}
		}
		return $f;
	}
	...
	public static function alert( $task , $message){
		if($task =='error') {
			$alert ='<div class="alert alert-danger  fade in block-inner"><button data-dismiss="alert" class="close" type="button"> x </button><i class="icon-cancel-circle"></i> '. $message.' </div>';
		} elseif ($task =='success') {
			$alert ='<div class="alert alert-success fade in block-inner"><button data-dismiss="alert" class="close" type="button"> x </button><i class="icon-checkmark-circle"></i> '. $message.' </div>';
		} elseif ($task =='warning') {
			$alert ='<div class="alert alert-warning fade in block-inner"><button data-dismiss="alert" class="close" type="button"> x </button><i class="icon-warning"></i> '. $message.' </div>';
		} else {
			$alert ='<div class="alert alert-info  fade in block-inner"><button data-dismiss="alert" class="close" type="button"> x </button><i class="icon-info"></i> '. $message.' </div>';
		}
		return $alert;
	}
	...
	public static function showUploadedFile($file,$path , $width = 50 , $height = 50) {
		$files =  public_path($path . $file) ;
		if(is_file($files ) && $file !="") {
		//	echo $files ;
			$info = pathinfo($files);
			if($info['extension'] == "jpg" || $info['extension'] == "jpeg" ||  $info['extension'] == "png" || $info['extension'] == "PNG" || $info['extension'] == "gif" || $info['extension'] == "JPEG" || $info['extension'] == "JPG"){
				$path_file = str_replace("./","",$path);
				return '<p style="margin:0px;"><a href="'.url( $path_file . $file).'" target="_blank" class="previewImage">
				<img src="'.asset( $path_file . $file ).'" border="0" width="'. $width .'" height="'.$height.'" class="b-r-100" /></a></p>';
			} else {
				$path_file = str_replace("./","",$path);
				return '<p style="margin:0px;"><a href="'.url( $path_file . $file).'" target="_blank" class="previewImage">
				<img src="'.asset( $path_file . $file ).'" border="0" width="'. $width .'" height="'.$height.'"  class="b-r-100" /></a></p>';
				...
				$path_files = str_replace("./","",$path);
				return '<p style="margin:0px;"><a href="'.url( $path_files . "no-image.png").'" target="_blank" class="previewImage">
				<img src="'.asset("images/operadores/no-avatar.jpg" ).'" border="0" width="'. $width .'" height="'.$height.'" class="b-r-100" /></a></p>';
		}
	}
	public static function showRelatedFile2($arch, $name , $idfile , $width = 50) {
		$files =  $arch;
		$basepath = realpath('./');
		//echo $basepath;
		$path_link = str_replace($basepath,"",$files);
		if(file_exists($files) && $arch!="") {

			$info = pathinfo($files);
			if($info['extension'] == "jpg" || $info['extension'] == "jpeg" ||  $info['extension'] == "png" || $info['extension'] == "gif" || $info['extension'] == "JPG")
			{
				$path_file = str_replace($basepath,"",$files);
			}elseif(file_exists($basepath . "/gthres/css/icons/". SiteHelpers::getIconExt(strtolower($info['extension'])) . ".png")) {
				$path_file = "/gthres/css/icons/". SiteHelpers::getIconExt(strtolower($info['extension'])) . ".png";
			}else{
				$path_file = '/uploads/images/no-image.png';
			}
		} else {
			$path_file = '/uploads/images/no-image.png';
		}
				return '<p class="item-file-list" style="border-radius:5px;"><a href="'.url($path_link).'" target="_blank" class="relatedFile " data-toggle="tooltip" title="'. $name. '">
				<img src="'.asset( $path_file ).'" border="0" class="img-related" /></a>'.$name.'
				</p>';

	}
public static function showRelatedFiledelete($arch, $name , $idfile ,$width = 20) {
		$files =  $arch;
		$basepath = realpath('./');
		//echo $basepath;
		$path_link = str_replace($basepath,"",$files);
		if(file_exists($files) && $arch!="") {
			$info = pathinfo($files);
			if($info['extension'] == "jpg" || $info['extension'] == "jpeg" ||  $info['extension'] == "png" ||  $info['extension'] == "PNG" || $info['extension'] == "gif" || $info['extension'] == "JPG" || $info['extension'] == "JPEG"){
				$path_file = str_replace($basepath,"",$files);
			}elseif(file_exists($basepath . "/mass/css/icons/". SiteHelpers::getIconExt(strtolower($info['extension'])) . ".png")) {
				$path_file = "/mass/css/icons/". SiteHelpers::getIconExt(strtolower($info['extension'])) . ".png";
			}else	{
				$path_file = '/images/no-image.png';
			}
		} else {
			$path_file = '/images/no-image.png';
		}
		return '<div class="item-file-list" style="height:200px;font-size:8px;background:transparent;"><div style="height:25px;overflow:hidden;">'.$name.'</div><a href="'.url($path_link).'" target="_blank" class="relatedFile " data-toggle="tooltip" title="'. $name. '"  style="height:150px;">
				<img src="'.asset( $path_file ).'" border="0" class="img-related"  /></a><input type="checkbox" name="idfile[]" value="' . $idfile . '" /></div>';
	}
	...
	public static function gridDisplay($val , $field, $arr, $fieldconfig = NULL) {
		$formato = (isset($fieldconfig['formato']))?$fieldconfig['formato']:null;
		if(isset($arr['valid']) && $arr['valid'] ==1){
			...
			if(count($Q) >= 1 )	{
				...
				$v= '';
				$v .= (isset($fields[0]) && $fields[0] !='' ?  (SiteHelpers::formatData($row->$fields[0],$formato)).' ' : '');
				$v .= (isset($fields[1]) && $fields[1] !=''  ? (SiteHelpers::formatData($row-> $fields[1],$formato)).' ' : '');
				$v .= (isset($fields[2]) && $fields[2] !=''  ? (SiteHelpers::formatData($row->$fields[2],$formato)).' ' : '');
				return $v;
			} else {
				return '';
			}
		} else {
			return SiteHelpers::formatData($val,$formato);
		}
	}
	public static function gridDisplayView($val , $field, $arr, $fieldconfig = NULL) {
		$arr = explode(':',$arr);
		$formato = (isset($fieldconfig['formato']))?$fieldconfig['formato']:null;
	...
	public static function BBCode2Html($text) {
			$emotion =  URL::to('mass/js/plugins/markitup/images/emoticons/');
			$text = trim($text);
			// BBCode [code]
	...
	static function formatData($val, $formato){
		if(is_null($formato) || empty($formato)){
			return $val;
		}elseif($formato['type']=='numero' || $formato['type']=='moneda'){
			if(!is_numeric($val)){
				return $val;
			}elseif($formato['type']=='numero'){
				return number_format($val, $formato['decimals'], $formato['sep_decimal'], $formato['sep_thousand']);
			}else{
				return $formato['pretext'] . number_format($val, $formato['decimals'], $formato['sep_decimal'], $formato['sep_thousand']) . $formato['postext'];
			}
		}elseif($formato['type']=='boleano'){
			$vals = explode("/", $formato['boolval']);
			$_resp = $vals[1];
			if($val === 1 || $val === "1" || $val === true || $val === "true")
				$_resp = $vals[0];
			return $_resp;
		}elseif($formato['type']=='fecha'){
			$_formato = (isset($formato['dateformat']))?$formato['dateformat']:'Y-m-d';
			$fecha = date_create($val);
			$_resp = date_format($fecha, $_formato);
			return $_resp;
		}
		return $val;
	}
	static function getIconExt($ext){
		$res = "otros";
		switch ($ext) {
			case 'doc': case 'docx': $res = "doc"; break;
			case 'xls': case 'xlsx': $res = "xls"; break;
			case 'txt': case 'rtf': $res = "txt"; break;
			case 'pdf': $res = "pdf"; break;
			case 'ppt': case 'pptx': case 'pptm': case 'pps': case 'ppsx': $res = "ppt"; break;
			case 'js': case 'html': case 'htm': case 'php': case 'css': $res = "html"; break;
			case 'emlx': case 'msg': case 'eml': $res = "email"; break;
		}
		return $res;
	}
	static function getFieldFormatoNumero(){
		return array('type'  => 'numero','decimals' => '2','sep_decimal' => '.','pretext' => '','sep_thousand'  => ',','postext' => '','boolval' => '','dateformat' => '');
	}
	static function getFieldFormatoMoneda(){
		return array('type' => 'moneda','decimals' => '2','sep_decimal' => '.','pretext' => '$ ','sep_thousand' => ',','postext' => ' MXN','boolval' => '','dateformat' => '' );
	}
	static function getFieldFormatoFecha(){
		return array('type' => 'fecha','decimals'  => '','sep_decimal' => '','pretext' => '','sep_thousand'  => '','postext' => '','boolval' => '','dateformat' => 'Y-m-d');
	}
}
